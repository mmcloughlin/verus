...
use syn_verus::{
    ...
    ReturnType, Signature, SignatureDecreases, SignatureInvariants, SignatureUnwind, Stmt, Token,
    ...
    fn visit_fn(
        ...
        if let Some(SignatureUnwind { token, when }) = unwind {
            if let Some((when_token, mut when_expr)) = when {
                ...
                stmts.push(Stmt::Semi(
                    Expr::Verbatim(
                        quote_spanned!(when_expr.span() => ::builtin::no_unwind_when(#when_expr)),
                    ),
                    Semi { spans: [when_token.span] },
                ));
            } else {
                stmts.push(Stmt::Semi(
                    Expr::Verbatim(quote_spanned!(token.span() => ::builtin::no_unwind())),
                    Semi { spans: [token.span] },
                ));
            }
        }
