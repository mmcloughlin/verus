use crate::ast::{
    ...
    SpannedTyped, Typ, TypDecoration, TypX, Typs, UnaryOp, UnaryOpr, UnwindSpec, VarAt,
...
use crate::def::{
    ...
    prefix_lambda_type, prefix_no_unwind_when, prefix_open_inv, prefix_pre_var, prefix_requires,
...
use crate::func_to_air::{FunctionSst, UnwindSst};
...
fn stm_to_stmts(ctx: &Ctx, state: &mut State, stm: &Stm) -> Result<Vec<Stmt>, VirErr> {
    ...
    let result = match &stm.x {
        StmX::Call { fun, resolved_method, mode, typ_args: typs, args, split, dest, assert_id } => {
            ...
            let callee_never_unwinds = match &func.x.unwind_spec {
                UnwindSpec::NoUnwind => true,
                UnwindSpec::Default => func.x.mode != Mode::Exec,
                UnwindSpec::NoUnwindWhen(_) => false,
            };
                ...
                && !ctx.checking_spec_preconditions()
            {
                ...
                let e2 = match &func.x.unwind_spec {
                    UnwindSpec::Default => Arc::new(ExprX::Const(Constant::Bool(false))),
        ...
        StmX::BreakOrContinue { label, is_break } => {
            ...
            let is_air_break = *is_break && !loop_info.spinoff_loop;
            let mut stmts: Vec<Stmt> = Vec::new();
...
pub(crate) fn body_stm_to_air(
    ...
    let FunctionSst { reqs, post_condition, mask_set, body: stm, local_decls, statics, unwind } =
        function_sst;
    ...
    let unwind_air = match unwind {
        ...
        UnwindSst::NoUnwindWhen(exp) => {
            let e = if is_bit_vector_mode {
                let bv_expr_ctxt = &BvExprCtxt::new();
                bv_exp_to_expr(ctx, &exp, bv_expr_ctxt)?
            } else {
                let expr_ctxt = &ExprCtxt::new_mode(ExprMode::Body);
                exp_to_expr(ctx, &exp, expr_ctxt)?
            };
