...
pub struct VisitorScopeEntry {
    pub typ: Typ,
    pub mutable: bool,
}

pub type VisitorScopeMap = ScopeMap<Ident, VisitorScopeEntry>;
...
fn insert_pattern_vars(map: &mut VisitorScopeMap, pattern: &Pattern, is_uninit: bool) {
    match &pattern.x {
        PatternX::Wildcard(_) => {}
        PatternX::Var { name, mutable } => {
            let entry =
                VisitorScopeEntry { typ: pattern.typ.clone(), mutable: *mutable || is_uninit };
            let _ = map.insert(name.clone(), entry);
        }
        PatternX::Tuple(ps) => {
            for p in ps.iter() {
                insert_pattern_vars(map, p, is_uninit);
            }
        }
        PatternX::Constructor(_, _, binders) => {
            for binder in binders.iter() {
                insert_pattern_vars(map, &binder.a, is_uninit);
            }
        }
        PatternX::Or(pat1, _) => {
            insert_pattern_vars(map, pat1, is_uninit);
            // pat2 should bind an identical set of variables
        }
    }
}

pub(crate) fn expr_visitor_check_with_scope_map<E, MF>(
...
{
    match expr_visitor_dfs(expr, scope_map, &mut |scope_map, expr| match mf(scope_map, expr) {
        Ok(()) => VisitorControlFlow::Recurse,
        Err(e) => VisitorControlFlow::Stop(e),
    }) {
        VisitorControlFlow::Recurse => Ok(()),
        ...
        VisitorControlFlow::Recurse => {
            match &expr.x {
                ...
                ExprX::Quant(_quant, binders, e1) => {
                    map.push_scope(true);
                    for binder in binders.iter() {
                        let entry = VisitorScopeEntry { typ: binder.a.clone(), mutable: false };
                        let _ = map.insert(binder.name.clone(), entry);
                    }
                    expr_visitor_control_flow!(expr_visitor_dfs(e1, map, mf));
                    map.pop_scope();
                }
                ...
                ExprX::ExecClosure { params, ret, requires, ensures, body, external_spec } => {
                    ...
                    map.push_scope(true);
                    let ret_entry = VisitorScopeEntry { typ: ret.a.clone(), mutable: false };
                    let _ = map.insert(ret.name.clone(), ret_entry);
                    ...
                    match external_spec {
                        ...
                        Some((cid, cexpr)) => {
                            map.push_scope(true);
                            let entry = VisitorScopeEntry { typ: expr.typ.clone(), mutable: true };
                            let _ = map.insert(cid.clone(), entry);
                ...
                ExprX::Match(e1, arms) => {
                    ...
                    for arm in arms.iter() {
                        map.push_scope(true);
                        insert_pattern_vars(map, &arm.x.pattern, false);
                ...
                ExprX::OpenInvariant(inv, binder, body, _atomicity) => {
                    ...
                    map.push_scope(true);
                    let entry = VisitorScopeEntry { typ: binder.a.clone(), mutable: true };
                    let _ = map.insert(binder.name.clone(), entry);
                    expr_visitor_control_flow!(expr_visitor_dfs(body, map, mf));
                    map.pop_scope();
                }
    ...
    MF: FnMut(&mut VisitorScopeMap, &Expr) -> VisitorControlFlow<T>,
{
    match &stmt.x {
        ...
        StmtX::Decl { pattern, mode: _, init } => {
            ...
            insert_pattern_vars(map, &pattern, init.is_none());
        }
    }
    VisitorControlFlow::Recurse
}
    ...
    MF: FnMut(&mut VisitorScopeMap, &Expr) -> VisitorControlFlow<T>,
{
    ...
    for p in params.iter() {
        let entry = VisitorScopeEntry { typ: p.x.typ.clone(), mutable: false };
        let _ = map.insert(p.x.name.clone(), entry);
    }
    ...
    let exprx = match &expr.x {
        ...
        ExprX::ExecClosure { params, ret, requires, ensures, body, external_spec } => {
            ...
            let external_spec = match external_spec {
                ...
                Some((cid, cexpr)) => {
                    map.push_scope(true);
                    let entry = VisitorScopeEntry { typ: expr.typ.clone(), mutable: false };
                    let _ = map.insert(cid.clone(), entry);
        ...
        ExprX::Match(e1, arms) => {
            ...
            let arms: Result<Vec<Arm>, VirErr> = vec_map_result(arms, |arm| {
                ...
                insert_pattern_vars(map, &pattern, false);
    ...
    FT: Fn(&mut E, &Typ) -> Result<Typ, VirErr>,
{
    ...
    for p in params.iter() {
        let entry = VisitorScopeEntry { typ: p.x.typ.clone(), mutable: p.x.is_mut };
        let _ = map.insert(p.x.name.clone(), entry);
    }
    ...
    if function.x.has_return() {
        let entry = VisitorScopeEntry { typ: ret.x.typ.clone(), mutable: false };
        let _ = map.insert(ret.x.name.clone(), entry);
    }
    let ensure =
