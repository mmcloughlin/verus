...
pub fn check_traits(krate: &Krate, ctx: &GlobalCtx) -> Result<(), VirErr> {
    ...
    //     trait T)
    //   - D: U --> D: T when there is a trait bound `trait U: T``
    //     (because in a trait_impl of a `trait U: T`, the methods that implement T also implement U)
    ...
    //   }
    // We can store a Dictionary_U inside Dictionary_T:
    //   struct Dictionary_T<Self, Y> {
    //     a: Dictionary_U<Self::Y>,
    //     f: Fn(x: Self, y: Self::Y) -> bool,
    //     g: Fn(x: Self, y: Self::Y) -> Self { requires(f(x, y)); },
    //   }
    // This adds an edge, like for bounds on trait parameters:
    //   - T --> U
    // This also ensures that whenever Self::Y is used in f and g,
    // the dictionary a: Dictionary_U<Self::Y> is available.
