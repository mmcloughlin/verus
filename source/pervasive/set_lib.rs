...
impl<A> Set<A> {
            ...
            self.len(),
    {
        ...
        if self.len() == 1 {
            ...
            assert(forall |elt: A| self.remove(x).contains(elt) && #[trigger] r(elt,y) ==> #[trigger] r(y,elt));
            assert forall |elt: A| self.contains(elt) && #[trigger] r(elt,min_updated) implies #[trigger] r(min_updated,elt) by {
            ...
            assert forall |min_poss: A| is_minimal(r, min_poss, self) implies self.find_unique_minimal(r) == min_poss by {
            ...
            self.len(),
    {
        ...
        if self.len() == 1 {
            ...
            assert forall |elt: A| self.contains(elt) && #[trigger] r(max_updated,elt) implies #[trigger] r(elt,max_updated) by {
            ...
            assert forall |max_poss: A| is_maximal(r, max_poss, self) implies self.find_unique_maximal(r) == max_poss by {
    ...
    {
        assert(is_maximal(r, max, self) ==> forall |x:A| !self.contains(x) || !r(max,x) || r(x,max));
    }
    ...
    {
        assert(is_minimal(r, min, self) ==> forall |x:A| !self.contains(x) || !r(x,min) || r(min,x));
    }
    ...
    {
        assert forall |min: A, min_prime: A| is_least(r, min, self) && is_least(r, min_prime, self) implies min == min_prime by {
    ...
    {
        assert forall |max: A, max_prime: A| is_greatest(r, max, self) && is_greatest(r, max_prime, self) implies max == max_prime by {
    ...
    pub proof fn lemma_minimal_is_unique(self, r: FnSpec(A,A) -> bool)
        requires
            total_ordering(r),
        ensures
            forall |min: A, min_prime: A| is_minimal(r, min, self) && is_minimal(r, min_prime, self) ==> min == min_prime,
    {
        assert forall |min: A, min_prime: A| is_minimal(r, min, self) && is_minimal(r, min_prime, self) implies min == min_prime by {
    ...
    pub proof fn lemma_maximal_is_unique(self, r: FnSpec(A,A) -> bool)
        requires
            ...
            total_ordering(r),
        ensures
            forall |max: A, max_prime: A| is_maximal(r, max, self) && is_maximal(r, max_prime, self) ==> max == max_prime,
    {
        assert forall |max: A, max_prime: A| is_maximal(r, max, self) && is_maximal(r, max_prime, self) implies max == max_prime by {
